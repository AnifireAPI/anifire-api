...
hindiApiRouter.get("/episode", async (c) => {
  ...
  if (!tmdbIdParam && !malId && !anilistId) {
    return c.json({ provider: "Anifire", status: 400, error: "Provide tmdbId, malId, or anilistId" }, 400);
  }

  if (type !== "movie" && (!season || !episode)) {
    return c.json({ provider: "Anifire", status: 400, error: "Missing season or episode" }, 400);
  }
  ...
  try {
    ...
    return c.json(
      { provider: "Anifire", status, error: msg.replace(/\[NOT_FOUND\]\s*/, '') },
      status
    );
  }
});
