...
toonStreamRouter.get("/home", async (c) => {
  ...
  try {
    ...
    if (!data) {
      return c.json({ provider: "Anifire", status: 404, error: "No data from ToonStream" }, 404);
    }

    return c.json({ provider: "Anifire", status: 200, data });
  } catch (error) {
    log.warn(`[ToonStream] Home route error: ${error}`);
    return c.json({ provider: "Anifire", status: 502, error: "ToonStream upstream unavailable" }, 502);
  }
});
...
toonStreamRouter.get("/episode/:slug", async (c) => {
  ...
  if (!slug) {
    return c.json({ provider: "Anifire", status: 400, error: "Missing episode slug" }, 400);
  }
  ...
  try {
    ...
    if (!data) {
      return c.json({ provider: "Anifire", status: 404, error: "Episode not found on ToonStream" }, 404);
    }
...
toonStreamRouter.get("/watch/:animeName", async (c) => {
  ...
  if (!animeName) {
    return c.json({ provider: "Anifire", status: 400, error: "Missing animeName" }, 400);
  }
