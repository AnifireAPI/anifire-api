...
anilistHindiRouter.get("/episode", async (c) => {
  ...
  if (!anilistId && !malId) {
    return c.json({ provider: "Anifire", error: "Missing anilistId parameter" }, 400);
  }
  if (!episodeStr) {
  ...
  try {
    ...
    if (!data) {
      return c.json({ provider: "Anifire", error: "No data found" }, 404);
    }
    ...
    return c.json(
      { provider: "Anifire", error: error?.message || "Internal error" },
      isNotFound ? 404 : 500
    );
  }
});
