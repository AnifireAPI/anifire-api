...
hindiDubbedRouter.get("/search", async (c) => {
    ...
    if (!title) {
        return c.json({ provider: "Anifire", status: 400, error: "Missing title parameter" }, 400);
    }
...
hindiDubbedRouter.get("/anime/:slug", async (c) => {
    ...
    if (!slug) {
        return c.json({ provider: "Anifire", status: 400, error: "Missing slug parameter" }, 400);
    }
...
hindiDubbedRouter.get("/", (c) => {
    return c.json({ provider: "Anifire",
        status: 200,
