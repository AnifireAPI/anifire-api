...
hianimeRouter.get("/home", async (c) => {
    ...
    return c.json({ provider: "Anifire", status: 200, data }, { status: 200 });
});
...
hianimeRouter.get("/episode/sources", async (c) => {
    ...
    try {
        ...
        return c.json({ provider: "Anifire", status: 404, message: "Sources not found or ID invalid", error: error.message }, { status: 404 });
    }
});
