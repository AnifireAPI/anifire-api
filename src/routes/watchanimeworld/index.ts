    ...
    if (SUPABASE_URL && AUTH_KEY) {
        try {
            ...
            const response = await fetchWithRetry(`${SUPABASE_URL}/functions/v1/watchanimeworld-scraper?episodeUrl=${parsed.slug}`, {
                ...
                body: JSON.stringify({ name: "AnifireAPI Proxy" })
            });
...
watchawRouter.get("/search", async (c) => {
    ...
    if (!query) return c.json({ provider: "Anifire", status: 400, error: "Missing q parameter" }, 400);
...
watchawRouter.get("/parse/:slug", (c) => {
    ...
    if (!parsed) return c.json({ provider: "Anifire", status: 400, error: "Invalid slug format" }, 400);
    return c.json({ provider: "Anifire", status: 200, data: parsed });
});
...
watchawRouter.get("/episode", async (c) => {
    ...
    if (!identifier) {
        return c.json({ provider: "Anifire", status: 400, error: "Missing id or episodeUrl parameter" }, 400);
    }
    ...
    return c.json({ provider: "Anifire", status: 200, data }, 200);
});
