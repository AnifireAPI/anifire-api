...
app.get(`${BASE_PATH}/docs/llm`, async (c) => {
    try {
        ...
        let fullContent = "Anifire API — FULL DOCUMENTATION\n\n";
        ...
        for (const file of files) {
            const possiblePaths = [
                ...
                path.join(process.cwd(), "src", "docs", file),
                path.join(process.cwd(), "dist", "src", "docs", file)
            ];
...
app.get("/docs/:section?", (c) => {
    ...
    // Load all documentation content server-side to embed in the HTML
    const safeSections = ["intro", "hianime", "anikai", "animeya", "sdk", "regional", "utility", "external"];
    const docsContent: Record<string, string> = {};
    
    for (const sec of safeSections) {
        const possiblePaths = [
            path.join(__dirname, "docs", `${sec}.md`),
            path.join(process.cwd(), "src", "docs", `${sec}.md`),
            path.join(process.cwd(), "dist", "src", "docs", `${sec}.md`)
        ];
        ...
        docsContent[sec] = content || `# Error\nDocumentation for **${sec}** could not be loaded.`;
    }

    // Generate LLM concatenated documentation
    let llmContent = "# Anifire API — FULL DOCUMENTATION\n\nNOTE: This document concatenates all documentation files. Each file is delimited with BEGIN/END markers to help LLM consumption.\n\n";
    for (const sec of safeSections) {
        if (docsContent[sec] && !docsContent[sec].startsWith("# Error")) {
            llmContent += `\n=== BEGIN FILE: ${sec}.md ===\n\n${docsContent[sec]}\n\n=== END FILE: ${sec}.md ===\n`;
        }
    }
    docsContent["llm"] = llmContent;
    ...
    return c.html(`
        ...
        <head>
            ...
            <title>Anifire API | Developer Documentation</title>
                    ...
                    <aside id="sidebar" class="fixed inset-y-0 left-0 w-72 border-r border-zinc-800 flex flex-col bg-[#050505] z-[100] lg:relative lg:translate-x-0 -translate-x-full transition-transform duration-300 lg:transition-none">
                        <div class="p-6 border-b border-zinc-800 flex items-center justify-between">
                            <a href="/" class="flex items-center gap-2 group whitespace-nowrap">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-black font-bold transform transition-transform group-hover:rotate-6">
                                    <img src="/public/anifire-logo.png" alt="Anifire Logo" class="w-full h-full object-contain">
                                </div>
                                <span class="font-bold text-lg tracking-tight">Anifire<span class="text-zinc-600 font-normal ml-0.5 uppercase text-xs">API</span></span>
                            </a>
                ...
                <header class="lg:hidden h-14 border-b border-zinc-800 bg-black/50 backdrop-blur-xl flex items-center px-4 justify-between z-50">
                    <button onclick="toggleMobileNav()" class="p-2 text-zinc-400">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"> <line x1="3" y1="12" x2="21" y2="12" /> <line x1="3" y1="6" x2="21" y2="6" /> <line x1="3" y1="18" x2="21" y2="18" /> </svg>
                            ...
                            <span class="font-bold text-sm tracking-tight"> Anifire Docs </span>
                                <div class="w-10"> </div>
                                    </header>
                    ...
                    <!--Populated by JS-->
                        ...
                        <div class="mt-12 space-y-4">
                            <div class="text-[10px] uppercase font-bold text-zinc-600 tracking-wider"> Community </div>
                                <a href="https://github.com/AnifireAPI/anifire-api" class="flex items-center gap-2 text-xs text-zinc-500 hover:text-white transition-colors">
                                                            ...
                                                            <span> Copied to clipboard </span>
                                                                ...
                                                                <script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"> </script>
                                                                    <script src="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/highlight.min.js"> </script>
                                                                        <script>
// Navigation Logic
const sections=['intro', 'hianime', 'anikai', 'animeya', 'regional', 'external', 'utility', 'sdk', 'llm'];
let currentSection='${sectionParam}';

// Embedded documentation content
const docsData = ${JSON.stringify(docsContent).replace(/</g, '\\u003c')};
...
async function loadSection(section) {
    ...
    setTimeout(() => {
        try {
            ...
            const content = docsData[section];
...
app.get("/api", (c) =>
    c.json({
        status: 200,
        provider: "Anifire",
        message: "🎌 Welcome to Anifire API!",
        version: pkgJson.version,
        endpoints: {
...
(function () {
    ...
    const serverInstance = serve({
        ...
        log.info(`🎌 Anifire API running at http://localhost:${serverPort}`);
