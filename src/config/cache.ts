...
export class AnifireCache {
    private static instance: AnifireCache | null = null;
    ...
    constructor() {
        ...
        this.memoryCache = new LRUCache<string, string>({
            ...
            ttl: AnifireCache.DEFAULT_TTL_SECONDS * 1000,
            updateAgeOnGet: true,
        });
    ...
    static getInstance(): AnifireCache {
        if (!AnifireCache.instance) {
            AnifireCache.instance = new AnifireCache();
        }
        return AnifireCache.instance;
    }

    async getOrSet<T>(
        dataGetter: () => Promise<T>,
        key: string,
        ttlSeconds: number = AnifireCache.DEFAULT_TTL_SECONDS
    ): Promise<T> {
    ...
    async close(): Promise<void> {
        ...
        AnifireCache.instance = null;
    }
}

export const cache = AnifireCache.getInstance();
