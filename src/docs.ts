...
Welcome to the **Anifire API** documentation. This API provides a unified interface for accessing anime, manga, light novels, and comics from various sources.
...
The quickest way to start using Anifire API is via \`fetch\` or \`curl\`.
...
\`\`\`
`,
  external: `# Classic External Scrapers
...
\`\`\`
`,
  hianime: `# HiAnime Scraper (AniWatch)
...
`,
  regional: `# Regional Scrapers (Hindi, Tamil, Telugu)
