export interface AnifireConfig {
    baseUrl?: string;
    apiKey?: string;
}

export class AnifireClient {
    private baseUrl: string;
    private apiKey?: string;

    constructor(config?: AnifireConfig) {
    ...
    public async request<T>(path: string, options?: RequestInit): Promise<T> {
        ...
        if (!response.ok) {
            throw new Error(`Anifire API Error: ${response.status} ${response.statusText}`);
        }
